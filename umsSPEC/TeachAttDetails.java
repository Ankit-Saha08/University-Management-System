...
public class TeachAttDetails extends JFrame{

	JTable tbl;
	JScrollPane jsp;
	
	String colNames[] = {"Emp ID", "Date", "1stt Half", "2nd Half"};
	...
	public TeachAttDetails() {
		super("SPEC - Teacher Attendance Details");
		ImageIcon icon1 = new ImageIcon(ClassLoader.getSystemResource("icons/icon15.png"));
	    ...
	    try{
	    	...
	    	pst = C.prepareStatement("select * from attendance_teacher");
	    	rs = pst.executeQuery();
	    	
	    	int i=0, j=0;
	    	while (rs.next()){
	    		rows[i][j++] = rs.getString("emp_id");
